...
const TabbedResults: React.FC<TabbedResultsProps> = ({ data, searchCriteria }) => {
  ...
  // Create summary header banner for Profile tab
  const createSummaryBanner = (fields: Array<[string, any]>) => {
    // Extract key contact and giving info
    const fieldMap = new Map(fields);
    const getName = () => {
      const first = fieldMap.get('First Name') || fieldMap.get('first_name') || '';
      const last = fieldMap.get('Last Name') || fieldMap.get('last_name') || '';
      return `${first} ${last}`.trim() || 'Not Available';
    };

    const getPhone = () => {
      const phone = fieldMap.get('Phone') || fieldMap.get('phone') || fieldMap.get('Phone Number') || '';
      return phone || 'Not Available';
    };

    const getEmail = () => {
      const email = fieldMap.get('Email') || fieldMap.get('email') || fieldMap.get('Email Address') || '';
      return email || 'Not Available';
    };

    const getAddress = () => {
      const street = fieldMap.get('Street') || fieldMap.get('street') || fieldMap.get('Street Address') || '';
      const city = fieldMap.get('City') || fieldMap.get('city') || '';
      const state = fieldMap.get('State') || fieldMap.get('state') || '';
      const zip = fieldMap.get('ZIP') || fieldMap.get('zip') || fieldMap.get('Zip Code') || '';
      const parts = [street, `${city}, ${state} ${zip}`.trim()].filter(p => p);
      return parts.join('\n') || 'Not Available';
    };

    const getConstituentId = () => {
      return fieldMap.get('Constituent ID') || fieldMap.get('constituent_id') || '';
    };

    const getLifetimeGiving = () => {
      const value = fieldMap.get('Lifetime Giving') || fieldMap.get('lifetime_giving') || '';
      return value || 'No transactions found';
    };

    const getLargestGift = () => {
      const value = fieldMap.get('Largest Gift') || fieldMap.get('largest_gift') || '';
      return value || 'No transactions found';
    };

    const getFirstGift = () => {
      const value = fieldMap.get('First Gift') || fieldMap.get('first_gift') || '';
      return value || 'No transactions found';
    };

    const getLatestGift = () => {
      const value = fieldMap.get('Latest Gift') || fieldMap.get('latest_gift') || '';
      return value || 'No transactions found';
    };
    ...
    return (
      ...
      >
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 2 }}>
          {/* Name */}
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Typography variant="caption" sx={{ opacity: 0.8, fontWeight: 600 }}>👤 Name:</Typography>
            <Typography variant="body2" sx={{ fontWeight: 600 }}>{getName()}</Typography>
          </Box>

          {/* Phone */}
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Typography variant="caption" sx={{ opacity: 0.8, fontWeight: 600 }}>📞 Phone:</Typography>
            <Typography variant="body2" sx={{ fontWeight: 600 }}>{getPhone()}</Typography>
          </Box>

          {/* Largest Gift */}
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Typography variant="caption" sx={{ opacity: 0.8, fontWeight: 600 }}>💰 Largest Gift:</Typography>
            <Typography variant="body2" sx={{ fontWeight: 600 }}>{getLargestGift()}</Typography>
          </Box>

          {/* Address */}
          <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1, gridColumn: '1' }}>
            <Typography variant="caption" sx={{ opacity: 0.8, fontWeight: 600, mt: 0.5 }}>🏠 Address:</Typography>
            <Typography variant="body2" sx={{ fontWeight: 600, whiteSpace: 'pre-wrap' }}>{getAddress()}</Typography>
          </Box>

          {/* Email */}
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Typography variant="caption" sx={{ opacity: 0.8, fontWeight: 600 }}>✉️ Email:</Typography>
            <Typography variant="body2" sx={{ fontWeight: 600 }}>{getEmail()}</Typography>
          </Box>

          {/* First Gift */}
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Typography variant="caption" sx={{ opacity: 0.8, fontWeight: 600 }}>🎁 First Gift:</Typography>
            <Typography variant="body2" sx={{ fontWeight: 600 }}>{getFirstGift()}</Typography>
          </Box>

          {/* Constituent ID */}
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Typography variant="caption" sx={{ opacity: 0.8, fontWeight: 600 }}>🆔 Constituent ID:</Typography>
            <Typography variant="body2" sx={{ fontWeight: 600 }}>{getConstituentId()}</Typography>
          </Box>

          {/* Lifetime Giving */}
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Typography variant="caption" sx={{ opacity: 0.8, fontWeight: 600 }}>💎 Lifetime Giving:</Typography>
            <Typography variant="body2" sx={{ fontWeight: 600 }}>{getLifetimeGiving()}</Typography>
          </Box>

          {/* Latest Gift */}
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Typography variant="caption" sx={{ opacity: 0.8, fontWeight: 600 }}>📅 Latest Gift:</Typography>
            <Typography variant="body2" sx={{ fontWeight: 600 }}>{getLatestGift()}</Typography>
          </Box>
        </Box>
      </Paper>
    );
